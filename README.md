...
> **~120 usuarios concurrentes** sobre modelos 70B FP4 o **~30 usuarios** sobre 200B.
...
> [discusión honesta en §1.3.3](#133-nvlink-en-rtx-blackwell).
...
El TPOT baja casi linealmente con el número de nodos (ideal para
...
4× concurrencia → 2.6× latencia, 3× prompt throughput. Excelente
...
El **4× DGX Spark** ofrece el **mejor coste por usuario concurrente**
(~$132/usuario) en el rango 25–120 usuarios. Por encima de ~120 usuarios, el **8× RTX PRO
6000** gana en TPS/usuario (4.4 vs 3.1) pero pierde en TCO. Por debajo de 25, sobra con
1× o 2× Spark.
...
Anthropic Claude 3.5 Sonnet se estima en **~175–300B** parámetros
activos y la familia Claude Opus entre **400B–700B**. El escalón de 500B representa el "techo de entrada" al
...
FP4 (≈ 335 GB). En esta sección se asume 500B parámetros activos cuantizados a FP4, sin
...
El **4× DGX Spark es el "punto de entrada" al mundo 500B**: es el
...
Para 500B FP4, el **rango de ventaja del 4× DGX Spark** es
...
El crossover Spark → RTX PRO se desplaza a la izquierda a medida
...
Como 500B es **inviable en FP8** sobre todos los racks analizados, la única "perilla"
relevante es **FP4 (NVFP4) vs FP4 estándar vs INT4 AWQ**:
...
el verdadero alivio viene de **MoE** (Mixtral 8×22B activa solo 39B / 500B totales, ver §4.4).
...
sistema evaluado que lo soporta en FP4.
...
nº de Sparks / config rack (eje X). Asumimos batch size = $U_{c,\max}$ (escenario "fully packed"):
...
### 3.7 Conclusión: 500B FP4

Para modelos de 500B parámetros en FP4, el espectro de viabilidad se reduce drásticamente.
1× y 2× DGX Spark son OOM (la memoria agregada de 192–384 GB no cubre los ~400 GB
requeridos). 4× Spark es marginal: ofrece solo 1.6 TPS/usuario para hasta 16 usuarios,
muy por debajo del umbral de fluidez conversacional. La primera arquitectura viable es
**8× RTX PRO 6000**, que entrega 4.7 TPS/usuario para 17–50 usuarios. 16× RTX PRO 6000
cubre el rango de 51–200 usuarios a 3.9 TPS/u.

Para despliegues de más de 200 usuarios concurrentes, solo el **GB300 NVL72** (o un
clúster equivalente) es factible, sosteniendo ~4.9 TPS/u incluso a 500 usuarios. Esto
implica que los modelos de 500B requieren una inversión en infraestructura de rack o
granja sustancialmente mayor que los de 70B o 200B, y su viabilidad económica depende
de que el TPS por usuario por encima de 15 no sea un requisito estricto.
...
Con prefix caching, el **TTFT efectivo** para 30
...
Si el cliente es un **despacho de abogados** o un
...
Las arquitecturas MoE cambian **fundamentalmente** los números presentados porque solo
...
Con FP8 KV, el **doble de usuarios concurrentes** caben en la
...
> 70B FP4, frente a 3.0 TPS del baseline. Esto es **108× más throughput** y pone al
...
Esta sección muestra tres gráficas, una por tamaño de modelo. Cada curva representa el
**TPS/usuario concurrente** que entrega cada arquitectura al aumentar la concurrencia.
El eje X es el número de usuarios concurrentes (carga de trabajo); el eje Y es el
throughput por usuario en tokens/segundo. Donde una curva cae por debajo de 15 TPS/u
(el umbral de fluidez conversacional), esa arquitectura deja de ser viable para ese
número de usuarios.
...
**Lectura:** El crossover con 1× Spark ocurre en **~50 usuarios**; con 2× Spark en
**~80**; con 4× Spark en **~110–140**. Por encima de eso, el rack RTX PRO es
...
**Lectura:** 1× y 2× Spark son OOM (línea 0). 4× Spark soporta hasta ~31 usuarios a 32K.
El crossover con 4× Spark ocurre en **~30 usuarios**; con 8× RTX PRO en **~50**.
El 16× RTX PRO es viable hasta ~200 usuarios.
...
**Lectura:** 1× y 2× Spark son OOM. 4× Spark solo soporta 1–16 usuarios y entrega
1.6 TPS/u (por debajo del umbral de 15). 8× RTX PRO 6000 es la primera arquitectura
que supera 15 TPS/u, cubriendo 17–50 usuarios. 16× RTX PRO 6000 cubre 51–200 usuarios
a 3.9 TPS/u. GB300 NVL72 es la única opción para >200 usuarios.
...
| Max modelo FP4 (paráms) | 200B | 400B | 512B | 700B | 1 800B | 3 700B | 36 000B |
| Max **70B FP4** U_c (32K) | ~16 | ~41 | ~66 | ~91 | ~141 | ~291 | ~4 700 |
| Max **200B FP4** U_c (32K) | OOM | ~11 | ~19 | ~31 | ~51 | ~111 | ~2 800 |
...
ARCHS = {
    ...
    "2× Spark":        {"H":  256, "r_base": 150, "eta": 0.92, "capex": 6_000},
    "3× Spark ring":   {"H":  384, "r_base": 215, "eta": 0.85, "capex": 9_000},
    "4× Spark switch": {"H":  512, "r_base": 280, "eta": 0.80, "capex": 12_000},
...
Mem. modelo: 35.0 GB | Mem. KV: 0.6 GB | Overhead: 8 GB
---------------------------------------------------------------------------
Arquitectura       Mem OK   TPS      TPS/u    $/u·y
1× Spark           ✓        75       2.50     $42
2× Spark           ✓        138      4.60     $83
3× Spark ring      ✓        183      6.10     $125
4× Spark switch    ✓        224      7.47     $167
8× RTX PRO 6000    ✓        589      19.63    $495
16× RTX PRO 6000   ✓        1044     34.80    $990
GB300 NVL72        ✓        7360     245.33   $34679
```

> **Interpretación.** Para este workload, el **1× DGX Spark** es la opción con mejor costo por
> usuario-año ($42) pero ofrece solo 2.5 TPS/u. El **16× RTX PRO 6000** ofrece el mejor TPS/u
> (34.8) a un costo ~24× superior por usuario-año.
